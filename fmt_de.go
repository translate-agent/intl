...
func seqDayWeekday(locale language.Tag, opts Options) *symbols.Seq {
	...
	case cldr.AR, cldr.SYR:
		seq.Add(weekday, symbols.TxtArabicComma, ' ', day)
	case cldr.AZ:
	...
	case cldr.FA:
		seq.Add(weekday, ' ', day, symbols.TxtPersianOrdinalSuffix)
	case cldr.FF:
		if script == cldr.Adlm {
			seq.Add(weekday, ' ', day)
		} else {
	...
	case cldr.HE:
		seq.Add(weekday, symbols.TxtHebrewHeDash, day)
	case cldr.HU:
		seq.Add(day, '.', ',', ' ', weekday)
	case cldr.II:
		if opts.Weekday == WeekdayNarrow {
			seq.Add(day, weekday, ',', ' ', symbols.Txtꑍ)
		} else {
			seq.Add(day, symbols.Txtꑍ, ',', ' ', weekday)
		}
	case cldr.JA:
		if opts.Weekday == WeekdayLong {
			seq.Add(day, symbols.Txt日, weekday)
		} else {
			seq.Add(day, symbols.Txt日, '(', weekday, ')')
		}
	case cldr.KK:
	...
	case cldr.KO:
		if opts.Weekday == WeekdayLong {
			seq.Add(day, symbols.Txt일, ' ', weekday)
		} else {
			seq.Add(day, symbols.Txt일, ' ', '(', weekday, ')')
		}
	case cldr.KS:
	...
	case cldr.KSH:
		seq.Add(weekday, symbols.TxtColognianDa, day, '.')
	case cldr.MN:
		seq.Add(symbols.Symbol_dd, '.', ' ', weekday)
	case cldr.MY:
		seq.Add(day, symbols.TxtBurmeseDay, weekday)
	case cldr.SD:
		if script == cldr.Deva {
			seq.Add(day, ' ', weekday)
		} else {
	...
	case cldr.VI:
		seq.Add(weekday, symbols.TxtVietnameseNgay, day)
	case cldr.YI:
		seq.Add(weekday, symbols.TxtYiddishDem, day, symbols.TxtYiddishTn)
	case cldr.ZH:
		if script == cldr.Hant {
			seq.Add(day, ' ', weekday)
		} else {
